...
<title>SHIPPING DATA ENTRY PAGE</title>
  ...
  <?php
    ...
    if (count($_REQUEST)!==0) {
      $state = $sql_task_manager->batch_opt_db_vali(array('COMBINED_SERIAL'), array("Electrical Serial"), 'ELECTRODE_SERIAL', 'LAMINATOR', 0, '')
      || $sql_task_manager->batch_opt_db_vali(array('COMBINED_SERIAL'), array("Electrical Serial"), 'COMBINED_SERIAL', 'SLITTER', 0, '');
    }
  ?>
